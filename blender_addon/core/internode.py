...
class PdInternode(PdPlantPart):
    ...
    def checkIfSeedlingLeavesHaveAbscissed(self):
        if self.leftLeaf is not None and self.leftLeaf.isSeedlingLeaf:
        ...
        if self.rightLeaf is not None and self.rightLeaf.isSeedlingLeaf:
            nodes = int(self.plant.pSeedlingLeaf.nodesOnStemWhenFallsOff)
            if self.plant.mainStemNodeCount() > nodes:
    ...
    def propFullLength(self):
        ...
        return getattr(p, "lengthAtOptimalFinalBiomassAndExpansion_mm", 0.0) * self.lengthExpansion * self.boltingExpansion

    def propFullWidth(self):
        p = self.plant.pInternode
        return getattr(p, "widthAtOptimalFinalBiomassAndExpansion_mm", 0.0) * self.widthExpansion
